...
UR10_STAGE_PATH = "/UR10"
UR10_USD_PATH = "/Isaac/Robots/UR10/ur10.usd"
...
if __name__ == "__main__":
    ...
    # Loading the ur10 robot USD
    ur10_asset_path = _nucleus_path + UR10_USD_PATH
    prim = _stage.DefinePrim(UR10_STAGE_PATH, "Xform")
    prim.GetReferences().AddReference(ur10_asset_path)
    ...
    # Load Joint State
    omni.kit.commands.execute(
        "ROSBridgeCreateJointState", path="/ROS_JointState", articulation_prim_rel=[UR10_STAGE_PATH], enabled=False
    )

    # Load Pose Tree
    omni.kit.commands.execute(
        "ROSBridgeCreatePoseTree", path="/ROS_PoseTree", target_prims_rel=[UR10_STAGE_PATH], enabled=False
    )
